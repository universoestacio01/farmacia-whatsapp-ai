...
export class MedicineSearchOrchestratorService {
  ...
  async searchMedicine(query: string): Promise<MedicineLookupSummary | null> {
    ...
    await this.requestLog?.record({ provider: "medicine_search", operation: "search_outcome", query,
      ...
      durationMs: Date.now() - startedAt, outcome: final.options.length ? ProviderRequestOutcome.SUCCESS : ProviderRequestOutcome.FAILED,
      failureReason: final.options.length ? undefined : `primary=${primary.searchStatus};final=${final.searchStatus};reason=${final.failureReason}` });
    this.logger.log(JSON.stringify({ event: "MEDICINE_SEARCH_OUTCOME", query, primaryStatus: primary.searchStatus,
      backupUnavailable: final.searchStatus === "backup_unavailable", finalStatus: final.searchStatus }));
    return final;
  }
  ...
  private async selectNormalized(
    ...
    const validOptions = options.filter((option) =>
      ["preco_popular", "openai_web"].includes(option.source) && Number.isFinite(option.salePrice) && (option.salePrice ?? 0) > 0 &&
    ...
    const mapped = validOptions.map((option, index) => {
      ...
      return {
        ...
        pricePolicy: option.source === "preco_popular" ? CATALOG_PRICE_POLICY : WEB_MEDICINE_PRICE_POLICY,
