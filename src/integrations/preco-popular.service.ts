...
export class PrecoPopularService {
  ...
  private readonly cache = new Map<
    string,
    { expiresAt: number; products: CatalogProduct[] }
  >();
  private readonly inFlight = new Map<string, Promise<CatalogProduct[]>>();
  ...
  async searchMedicines(query: string): Promise<NormalizedMedicineOption[]> {
    ...
    const products = await this.searchCatalog(term);
    return products
      .filter((product) => product.isMedicine)
      .map((product) => {
        const text = this.normalize(product.name);
        const dosagePattern =
          /\b\d+(?:[,.]\d+)?\s*(?:mcg|mg|g)\b(?:\s*\/\s*(?:\d+(?:[,.]\d+)?\s*)?(?:ml|g)\b)?/g;
        const dosage =
          text.match(dosagePattern)?.join(" + ").replace(/\s+/g, "") ||
          undefined;
        const packagingText = text.replace(dosagePattern, " ");
        const units = packagingText.match(
          /\b(\d+)\s*(?:comprimidos?|capsulas?|drageas?|unidades?|supositorios?)\b/,
        );
  ...
  async searchRetail(query: string): Promise<NormalizedRetailProduct[]> {
    const products = await this.searchCatalog(this.cleanRetailQuery(query));
    return products
      .filter((product) => !product.isMedicine)
  ...
  private async searchCatalog(
    ...
    const request = this.fetchCatalog(term, queryType)
      .then((products) => {
        ...
        this.cache.set(key, {
          products,
          expiresAt: Date.now() + (products.length ? 300_000 : 30_000),
        });
        return products;
      })
  ...
  private async fetchCatalog(
    ...
    try {
      // At most two pages, sharing one timeout. No detail request for each SKU.
