...
async function run() {
  ...
  try {
    ...
    check(/OpenAI.*busca web/.test(providerCards) && /gpt-5-mini/.test(providerCards), "OpenAI web backup configuration is visible");
    check(!/PharmaDB|BulAPI|Cosmos|Desconto/.test(providerCards), "Legacy catalogs and the old discount remain retired");
