...
A OpenAI retorna apenas URLs; a mensagem ao cliente e definida em `whatsapp-copy.ts`. Sem produto/oferta validada (`not_found`, `search_unverified`, `offer_unavailable`), ou com apresentacao restrita/dados inseguros, a mensagem e: "Esse produto nao esta disponivel para pedido no momento. Voce gostaria de buscar outro produto?". Nao afirma falta de estoque proprio e nao apresenta menu de atendimento humano. O estado `CATALOG_UNAVAILABLE` aceita sim, nao, tentar novamente ou diretamente outro nome, mantendo o carrinho. Falhas tecnicas usam "Nao consegui consultar esse produto agora. Voce gostaria de buscar outro produto?". O erro detalhado continua no log.

O encaminhamento humano de catalogo foi removido. Estados antigos `CATALOG_HELP_OPTIONS`, `CATALOG_REVIEW_REQUESTED` e `CATALOG_REVIEW_HANDLED` sao recuperados automaticamente na proxima mensagem; a antiga escolha 1 refaz a busca em vez de chamar atendente. Fotos voltam ao fluxo de identificacao/confirmacao, sem pausa humana. A fila especifica do painel foi retirada, mas a confirmacao manual de Pix e os comandos administrativos existentes foram preservados. Nenhum novo campo ou migracao de banco.
